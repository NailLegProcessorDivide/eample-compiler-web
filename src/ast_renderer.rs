...
use dioxus::prelude::*;
use sample_compiler_lib::{source_ast::{Prog, Func, VarDec, self, Exp, Stmt, id_to_string, Typ, TypedExp}, tokens};
...
pub fn RenderStmt<'a>(cx : Scope<'a, StmtCtx<'a>>) -> Element {
    ...
    match cx.props.stmt {
        ...
        Stmt::Stmts(stmts) => {
            let stmts = stmts.iter().map(|s| rsx! {RenderStmt {stmt : s, indent: cx.props.indent + 1}});
            cx.render( rsx! {
                "{pad}{{\n",
                stmts,
                "{pad}}}\n"
...
fn RenderFunc<'a>(cx : Scope<'a, FuncCtx<'a>>) -> Element {
    ...
    let params : String = cx.props.func.params.iter().map(|(id, t)| source_ast::id_to_string(id) + " " + &source_ast::typ_to_string(t) + ", ").collect();
    let locals = cx.props.func.locals.iter().map(|local| rsx! {RenderVar {var_dec : &local, indent : 1}});
    let body = cx.props.func.body.iter().map(|stmt| rsx! {RenderStmt {stmt : &stmt, indent : 1}});
    cx.render( rsx! {
...
pub fn RenderProg<'a>(cx : Scope<'a, ProgramCtx<'a>>) -> Element {
    let vars = cx.props.inp.globals.iter().enumerate().map(|(idx, v)| rsx! {RenderVar{var_dec : &v, indent : 0} });
    let funcs = cx.props.inp.funcs.iter().enumerate().map(|(idx, f)| rsx! {div{ key: "{idx}", RenderFunc {func : &f}}});
    cx.render( rsx! {
        div {
            vars
        }
